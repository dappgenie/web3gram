  ...
  <Teleport to="body">
    <Transition name="modal">
      <div
        ...
        @click="emit('backdrop')"
      >
        <div ref="target" class="modal-content" @click="$event.stopPropagation()">
          <div z-20 w-full sticky top-0 flex justify-between items-center>
            <div v-if="!hideTitle" text-sm font-bold text-color-primary px-4 h-10 md:h-14>
              <slot name="title" />
            </div>
            <button
              v-if="!hideClose"
