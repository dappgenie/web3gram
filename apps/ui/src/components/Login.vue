...
<template>
  <ModalCustom :show="isOpen" @close="closeTab()">
    <template #content>
      <div class="px-7">
        <div class="flex justify-start items-center">
          <img class="w-10 h-10" :src="logo" alt="">
          <div ml-4>
            <div class="text-color-secondary text-xl font-bold">
              Sign in
            </div>
        ...
        <div class="py-6">
            ...
            <ul class="py-4 flex justify-start gap-x-6">
              <li>
                <button type="button" class="background p-1 rounded-full" @click="socialLogin.socialLogin('google')">
                  <img
                    w-10 h-10 src="https://images.web3auth.io/login-google.svg" height="auto" width="auto"
                    alt="login-google"
                  >
                </button>
              </li>
              <li>
                <button type="button" class="background p-1 rounded-full" @click="socialLogin.socialLogin('facebook')">
                  <img
                    w-10 h-10 src="https://images.web3auth.io/login-facebook.svg" height="auto" width="auto"
                    alt="login-facebook"
                  >
                </button>
              </li>
          ...
          <div class="border-t border-color py-6">
            <FormTextInput
              id="biconomy-email-input"
              v-model="email"
              text-sm name="biconomy-email-input" label="Email"
              placeholder="Enter your email"
            />
            <Button id="biconomy-email-input-btn" w-full name="biconomy-email-input-btn" mt-4 color="base" rounded="none" @click="handleEmailSubmit">
              <template #content>
                Continue with Email
              </template>
            </Button>
          ...
          <div class="border-t border-color py-6">
            <div class="">
              <div text-primary font-bold class="w3a-group__title">
                EXTERNAL WALLET
              </div>
              <Button id="biconomy-email-input-btn" w-full name="biconomy-email-input-btn" mt-4 color="base" rounded="none" @click="socialLogin.metamaskLogin()">
                >
                <template #content>
                  Connect using MetaMask
                </template>
              </Button>
              <Button id="biconomy-email-input-btn" w-full name="biconomy-email-input-btn" mt-4 color="base" rounded="none" @click="socialLogin.walletConnectLogin()">
                <template #content>
                  Use Wallet Connect
                </template>
              </Button>
