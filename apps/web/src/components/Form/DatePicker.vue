...
const startValue2 = ref<any[]>([
    new Date().getDate(),
    DAYS[new Date().getDay()],
    MONTHS[new Date().getMonth()],
])
// const startValue2 = ref<any[]>(['Start Date'])
const endValue2 = ref<any[]>([
    new Date().getDate(),
    DAYS[new Date().getDay()],
    MONTHS[new Date().getMonth()],
])
...
const handleUpdate = (value: Date[]) => {
    ...
    startValue2.value = [
        value[0]?.getDate(),
        DAYS[value[0]?.getDay()],
        MONTHS[value[0]?.getMonth()],
    ]
    endValue2.value = [
        value[1]?.getDate(),
        DAYS[value[1]?.getDay()],
        MONTHS[value[1]?.getMonth()],
    ]

    dateValue.value =
        startValue.value === 'Select Date' && endValue.value === 'Select Date'
            ? startValue.value
            : props.single
            ? startValue.value
            : value.length === 2
        ...
        >
            <template #default>
                <div v-if="type === 'multi'" flex gap-x-6>
                    <div class="border-r-1 w-full border-blue">
                        ...
                        <div flex justify-start items-center>
                            ...
                            <button
                                :id="`${id}-date-picker-trigger`"
                                type="button"
                                class="date-picker-trigger w-full text-left"
                                :name="`${name}-date-picker-trigger`"
                                @click="isOpen = true"
                            >
                                <div
                                    text-sm
                                    :class="{ 'has-value': !!modelValue }"
                                    class="date-picker-trigger-text"
                                >
                                    {{ startValue }}
                                </div>
                            </button>
                ...
                <FormInputGroup v-if="type === 'single'" :is-label="true">
                    <template #label> Check-in - Check-out </template>
                    <template #left>
                ...
                <div
                    v-if="type === 'normal'"
                    class="flex w-full justify-center items-center gap-x-16 md:gap-x-28"
                >
                        ...
                        <div flex justify-center items-center>
                            ...
                            >
                                <div flex justify-start>
                                    <div
                                        :class="{ 'has-value': !!modelValue }"
                                        class="date-picker-trigger-text pe-2 text-[39px]"
                                    >
                                        {{ startValue2[0] }}
                                    </div>
                                    <div
            ...
            <template #content>
                <FormCalendar
                    ...
                    :options="{
                        monthsToShow: single ? 1 : 2,
                        mode: single ? 'single' : 'range',
                    }"
                    :model-value="modelValue"
                    :name="`${id}-date-picker-calendar`"
