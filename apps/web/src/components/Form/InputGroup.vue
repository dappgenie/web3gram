...
<template>
    <div
        class="input-grp"
        :class="[
            outline ? `outline-${outline}` : 'outline-all',
            outline === 'none' ? 'block' : 'flex',
        ]"
    >
        <div
            v-if="isLabel"
            class="label"
            :class="
                outline === 'none' ? 'relative' : 'absolute -top-2 left-2 px-1'
            "
        >
            <slot name="label"> Label </slot>
        </div>
        <div flex items-center gap-x-2 w-full>
            <slot name="left" />
            <slot />
            <slot name="right" />
        </div>
