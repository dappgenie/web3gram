...
import {
  ...
  subHours, subDays, subMonths, subYears,
...
<template>
  <div :id="id" :name="name" class="calendar-container">
    ...
    <div class="flex space-x-4 flex-1">
      <div v-if="props.options?.mode !== 'single'" :id="`${id}-shorthands`" :name="`${name}-shorthands`"
        class="calendar-shorthands">
        <div @click="setSelected(shortHand.value)" v-for="shortHand in shortHands" class="calendar-shorthand">
          {{ shortHand.name }}
        </div>
      </div>
      <div class="space-y-4">
        <div class="flex items-center space-x-4">
          <template v-for="(_, idx) in Array.from({ length: props.options?.monthsToShow ?? 2 })">
            <FormTextInput :id="`${id}-date-${idx}`" :name="`${name}-date-${idx}`"
              :modelValue="selected?.[idx] ? format(selected[idx], 'dd MMM yyyy') : ''" class="flex-1 w-0"
              :readonly="true">
              <template #left>
                <div text-xl text-primary i-mdi:calendar-month />
              </template>
            </FormTextInput>
            <FormTimeInput :disabled="selected.length < (props.options?.mode === 'single' ? 1 : 2)"
              :modelValue="selected?.[idx]" @update:modelValue="updateTime(idx, $event)" :id="`${id}-start-time`"
              :name="`${name}-end-time`" />
          </template>
        </div>
        <div :id="`${id}-calendar`" :name="`${name}-calendar`" class="calendar">
          <div v-for="(month, midx) in months" class="calendar-month">
            <div :id="`${id}-calendar-header`" :name="`${name}-calendar-header`" class="calendar-header">
              <button v-bind="buttonProps" v-if="midx === 0" class="calendar-header-icon"
                :id="`${id}-calendar-header-prev`" :name="`${name}-calendar-header-prev`" @click="prev">
                <div text-xl text-primary i-mdi:chevron-left />
              </button>
              <button v-bind="buttonProps" class="calendar-header-item" :id="`${id}-calendar-header-item`"
                :name="`${name}-calendar-header-item`">
                {{ month.name }} {{ month.year }}
              </button>
              <button v-bind="buttonProps" v-if="midx === (props.options?.monthsToShow ?? 1) - 1"
                class="calendar-header-icon" :id="`${id}-calendar-header-next`" :name="`${name}-calendar-header-next`"
                @click="next">
                <div text-xl text-primary i-mdi:chevron-right />
              </button>
            </div>
            <div class="calendar-week-days" :id="`${id}-calendar-week-days`" :name="`${name}-calendar-week-days`">
              <div :id="`${id}-calendar-week-day-${day}`" :name="`${name}-calendar-week-day-${day}`"
                class="calendar-week-day" v-for="day in weekDays">
                {{ day }}
              </div>
            </div>
            <div :id="`${id}-calendar-days`" :name="`${name}-calendar-days`" class="calendar-days">
              <div :id="`${id}-calendar-day-${day.dayId}`" :name="`${name}-calendar-day-${day.dayId}`"
                :class="getDayClasses(day)" class="calendar-day" v-for="day in month.days">
                <button v-bind="buttonProps" @click="selectDate(day.date)" :id="`${id}-calendar-day-${day.dayId}-text`"
                  :name="`${name}-calendar-day-${day.dayId}-text`" class="calendar-day-text">
                  {{ format(day.date, "dd") }}
                </button>
              </div>
            </div>
    ...
    <div class="flex items-center justify-end flex-none">
      <Button :id="`${id}-submit`" :name="`${name}-submit`" @click="submit" class="flex-none"
